...
func HandleFlutterInstall(apiKey, projectID string) {
	...
	if err := UpdateMainDart(projectRoot, projectID, apiKey); err != nil {
		...
		logx.Log().Indent(6).Code().Println(err.Error())
		fmt.Println("⚠️  Please manually add the following to your main.dart:")
		fmt.Println("   import 'package:firebase_core/firebase_core.dart';")
		fmt.Println("   import 'package:clix_flutter/clix_flutter.dart';")
		fmt.Println("   // Add Firebase.initializeApp() and Clix.initialize() in main() before runApp()")
	} else {
	...
	fmt.Println("2. Select Runner target > Signing & Capabilities")
	fmt.Println("3. Add 'Push Notifications' capability")
	fmt.Println("4. Add 'Background Modes' capability")
	...
	fmt.Println("1. Configure iOS push notifications in Xcode (as shown above)")
	fmt.Println("2. Upload your iOS Service Account Key to Clix console")
	fmt.Println("3. Run 'flutter run' to test your app")
	fmt.Println("4. Run 'clix doctor --flutter' to verify your setup")
	fmt.Println("==================================================")
}
